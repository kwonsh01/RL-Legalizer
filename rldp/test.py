...
class PPO(nn.Module):
    ...
    def make_batch(self):
        s_lst, r_lst, s_prime_lst, prob_a_lst, done_lst = [], [], [], [], []

        for transition in self.data:
            s, r, s_prime, prob_a, done = transition
            s_lst.append(s)
            r_lst.append([r])
        ...
        self.data = []
        return s, r, s_prime, done_mask, prob_a

    def train_net(self):
        s, r, s_prime, done_mask, prob_a = self.make_batch()
    ...
    while not timeUp:
        ...
        while not done:
            ...
            while not gcell_done:
                ...
                probf_train = torch.tensor(probf, dtype=torch.float).to(device)

                for i in range(probf.shape[0]):
                ...
                model.put_data((s, r, s_prime, probf_train, done))
